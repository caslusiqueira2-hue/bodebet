...
export function useMinesGame(userId: string) {
  ...
  const revealTile = useCallback((id: number) => {
    ...
    setTiles(prev => prev.map(t => t.id === id ? { ...t, isRevealed: true } : t));
    ...
    if (tile.isMine) {
      ...
      if (newRevealedCount === TOTAL_TILES - minesCount) {
        ...
        setGameState('WON');
        revealAllMines();
        // Overwrite safeTilesRevealed for history log correctly using functional state
        setSafeTilesRevealed(newRevealedCount);
        setTimeout(() => {
            // Need to use latest multiplier for saving history in WON state, handled by timeout or direct calculate
            ...
            setStats(prev => ({
                ...
                totalGames: prev.totalGames + 1,
                wins: prev.wins + 1,
                maxMultiplier: Math.max(prev.maxMultiplier, finalMultiplier),
                maxPayout: Math.max(prev.maxPayout, payout),
                totalProfit: prev.totalProfit + profit
            }));
        }, 0);
  ...
  }, [gameState, tiles, safeTilesRevealed, minesCount, bet, balance, setBalance, setHistory, setStats, userId]);
